...
def schedule_return():
    ...
    schedule_newer = api.get_schedule(league_id=[int(lck_id)], pageToken=newer_page_id)['schedule']['events']
    ...
    for i in all_schedule:
        if int(i['startTime'][:4]) >= 2022:
            ...
            hour,min = i['startTime'][11:16].split(":")
            checker = i['startTime'].replace("T","-")[:-1]
            # print(checker)
            record = [team[0]['name'],team[1]['name'],i['startTime'][:-1],checker]

            final_games_total.append(record)
            result = sorted(final_games_total,key= lambda date: datetime.strptime(date[-1], "%Y-%m-%d-%H:%M:%S"))
            # print(result)
            pass
